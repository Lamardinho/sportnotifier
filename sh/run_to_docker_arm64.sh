...
echo && echo RUN: && sudo docker run -it --name sportnotifier -p 8090:8090 -e SPRING_PROFILES_ACTIVE=prod,datasource_h2,ssl -d --network sportnotifier_network --restart always slezkin23/sportnotifier:arm64
