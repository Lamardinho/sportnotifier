...
echo && echo RUN: && sudo docker run -it --name sportnotifier -p 8090:8090 -e SPRING_PROFILES_ACTIVE=datasource_h2,prod,ssl -d --network sportnotifier_network --restart always slezkin23/sportnotifier:arm64
