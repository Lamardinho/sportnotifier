...
docker run -d --network $NETWORK_NAME --restart always --name $CONTAINER_NAME -e POSTGRES_USER=$POSTGRES_USER -e POSTGRES_PASSWORD=$POSTGRES_PASSWORD -p $PORT:5432 postgres:$POSTGRES_V
sleep 5
# Проверка успешного запуска контейнера
if [ $? -eq 0 ]; then
    echo "Контейнер PostgreSQL успешно запущен на порту $PORT."
